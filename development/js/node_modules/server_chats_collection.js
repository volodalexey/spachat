...
Server_chat_collection.prototype = {
    ...
    checkOrGenerateChatId: function(options, callback) {
        ...
        if (_this.checkIfExist(options.messageData)) {
            if (options.currentAttempt < options.generateAttempts) {
                ...
                options.messageData.chat_description.chatId = id_Generator.generateId();
    ...
    applyDeviceId: function(currentWebSocketConnection, parsedMessageData) {
        if (parsedMessageData.deviceId) {
            currentWebSocketConnection.deviceId = parsedMessageData.deviceId;
        }
        if (parsedMessageData.tempDeviceId) {
    ...
    onDeviceCreateChat: function(currentWebSocketConnection, parsedMessageData) {
        ...
        var serverChatInstance = _this.checkIfExist(correctedClientChatData);
        if (!serverChatInstance) {
            serverChatInstance = _this.storeNewChat(correctedClientChatData.chat_description);
        }
        ...
        if (!parsedMessageData.deviceId && !parsedMessageData.tempDeviceId) {
            currentWebSocketConnection.tempDeviceId = id_Generator.generateId(); // temp device id until the device send offer
        }
        serverChatInstance.storeWebSocketConnection(currentWebSocketConnection);
        var responseData = {
            type: 'chat_created',
            ...
            chat_description: serverChatInstance.toChatDescription()
        };
    ...
    onDeviceChatJoin: function(currentWebSocketConnection, parsedMessageData) {
        var _this = this;
        var serverChatInstance = _this.checkIfExist(parsedMessageData);
        if (!serverChatInstance) {
            serverChatInstance = _this.storeNewChat(parsedMessageData.chat_description);
        }
        ...
        var responseData = {
            ...
            tempDeviceId: currentWebSocketConnection.tempDeviceId,
            deviceId: currentWebSocketConnection.deviceId,
            connectedDevices: serverChatInstance.getAllWebSocketsConnections(),
            chat_description: serverChatInstance.toChatDescription(),
            restore_chat_state: parsedMessageData.restore_chat_state
        };

        serverChatInstance.broadcastMessage(JSON.stringify(responseData));
    ...
    onDeviceChatOffer: function(currentWebSocketConnection, parsedMessageData) {
        var _this = this, serverChatInstance = _this.checkIfExist(parsedMessageData);
        if (!serverChatInstance) {
        ...
        var toWebSocketConnection = serverChatInstance.getConnection(parsedMessageData.toDevice);
        ...
        var responseData = {
            ...
            toDevice: serverChatInstance.getAllConnectionDeviceId(toWebSocketConnection),
            chat_description: serverChatInstance.toChatDescription()
        };
